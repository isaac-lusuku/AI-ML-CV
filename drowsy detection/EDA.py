...
# Define the dataset directory (change to your local dataset path)
dataset_path = "/kaggle/input/driver-drowsiness-dataset-ddd/Driver Drowsiness Dataset (DDD)"  # e.g., "./dataset"

# Create a dictionary to store image file paths for each class
classes = ['Drowsy', 'Non Drowsy']
data_dict = {class_name: glob.glob(os.path.join(dataset_path, class_name, "*.*")) for class_name in classes}

# Print out how many images per class
for class_name, files in data_dict.items():
    print(f"{class_name}: {len(files)} images")



# Create a list of dictionaries to build a DataFrame
data_list = []
for class_name, files in data_dict.items():
    for f in files:
...
# Plotting the image intensity stats
